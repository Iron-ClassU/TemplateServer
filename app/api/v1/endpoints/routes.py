...
from app.schemas.route import RouteCreate, RouteInDB, RouteUpdate
...
async def create_route(
    route: RouteCreate, session: AsyncSession = Depends(get_session)
):
    """Create a new route."""
    route_service = RouteService(session)
    try:
        handler_config = (
            route.handler_config or {}
...
async def validate_route(route_id: int, session: AsyncSession = Depends(get_session)):
...
async def update_route_status(
    route_id: int, is_active: bool, session: AsyncSession = Depends(get_session)
):
    """Update route active status."""
    route_service = RouteService(session)
    try:
        updated_route = await route_service.update_route(
            route_id=route_id, is_active=is_active
        )
        if not updated_route:
            raise HTTPException(status_code=404, detail="Route not found")
        return {
            "message": f"Route status updated to {'active' if is_active else 'inactive'}"
        }
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
